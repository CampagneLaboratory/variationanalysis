...
public class AvroVariationParquetReader implements Closeable {
    ...
    String path;
    ParquetReader<PosRecord> pqReader;

    public AvroVariationParquetReader(String path) {
        this.path = path;
        AvroParquetReader.Builder<PosRecord> pqBuilder = AvroParquetReader.builder(new Path(path));
        Configuration conf = new Configuration();
        conf.set("parquet.avro.class", PosRecord.class.getCanonicalName());
