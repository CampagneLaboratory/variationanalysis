...
joined<-inner_join(correct,wrong, by=c("feature" = "feature", "sumCounts" = "sumCounts"))
head(joined,n=10)
...
View(head(sorted,n=40))

a<-joined%>% group_by(feature) %>%
       mutate(meanCorrect=mean.x,
                           meanWrong=mean.y,
                           diff=abs(meanWrong-meanCorrect)) %>% arrange(desc(diff))
