 ...
 */
public class NumDistinctAlleleAssembler8Layers implements ComputationGraphAssembler {
    ...
    public ComputationGraph createComputationalGraph(DomainDescriptor domainDescriptor) {
        ...
        ComputationGraphConfiguration.GraphBuilder build = graphConfiguration
                ...
                .addLayer("dense5", new DenseLayer.Builder().nIn((int) (numHiddenNodes * Math.pow(reduction, 3))).nOut((int) (numHiddenNodes * Math.pow(reduction, 4)))
                        ...
                        .activation("relu")
                        .build(), "dense4")
                .addLayer("dense6", new DenseLayer.Builder().nIn((int) (numHiddenNodes * Math.pow(reduction, 4))).nOut((int) (numHiddenNodes * Math.pow(reduction, 4)))
                        .weightInit(WEIGHT_INIT).learningRateDecayPolicy(learningRatePolicy)
                        .activation("relu")
                        .build(), "dense5")
                .addLayer("dense7", new DenseLayer.Builder().nIn((int) (numHiddenNodes * Math.pow(reduction, 4))).nOut((int) (numHiddenNodes * Math.pow(reduction, 4)))
                        .weightInit(WEIGHT_INIT).learningRateDecayPolicy(learningRatePolicy)
                        .activation("relu")
                        .build(), "dense6")
                .addLayer("dense8", new DenseLayer.Builder().nIn((int) (numHiddenNodes * Math.pow(reduction, 4))).nOut((int) (numHiddenNodes * Math.pow(reduction, 4)))
                        .weightInit(WEIGHT_INIT).learningRateDecayPolicy(learningRatePolicy)
                        .activation("relu")
                        .build(), "dense7");
        build.addLayer("numDistinctAlleles", new OutputLayer.Builder(
                ...
                .nOut(domainDescriptor.getNumOutputs("numDistinctAlleles")[0]).build(), "dense8").addInputs();
        ...
        for (int i = 1; i < outputNames.length; i++) {
            build.addLayer(outputNames[i], new OutputLayer.Builder(
                    ...
                    .weightInit(WEIGHT_INIT)
                    .activation("softmax").weightInit(WEIGHT_INIT).learningRateDecayPolicy(learningRatePolicy)
                    .nIn((int) (numHiddenNodes * Math.pow(reduction, 4))).nOut(domainDescriptor.getNumOutputs(outputNames[i])[0]).build(), "dense8");
        }
