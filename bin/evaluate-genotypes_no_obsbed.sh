...
if [ ! "${NUM_ARGS}" == "${NUM_ARGS_EXPECTED}" ]; then
   ...
   echo "The 3rd argument is optional. You can bypass the predict phase by defining the variable VCF_OUTPUT to point to the output of predict"
   exit 1;
fi
MODEL_DIR=$1
MODEL_PREFIX=$2
...
if [ -z "${VCF_OUTPUT+set}" ]; then
    echo "VCF_OUTPUT is not defined. Running predict for ${DATASET_SBI}."
    ...
    echo "Evaluating with VCF_OUTPUT=${VCF_OUTPUT}"
fi
